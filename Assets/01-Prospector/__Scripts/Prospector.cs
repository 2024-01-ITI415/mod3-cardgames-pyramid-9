...
public class Prospector : MonoBehaviour {
    ...
    void Awake(){
        S = this;
        discardPile = new List<CardProspector>(); // Initialize discardPile list
    }
    ...
    layout = GetComponent<Layout>();    // Initialize the layout
    layout.ReadLayout(layoutXML.text);    // Pass LayoutXML to it

    Card c;    
    for (int cNum = 0; cNum < deck.cards.Count; cNum++) {    // Loop through each card in the deck
        c = deck.cards[cNum];    
    ...
    List<CardProspector> ConvertListCardsToListCardProspectors(List<Card> lCD) {    
        ...
        foreach (Card tCD in lCD) {    
            tCP = tCD as CardProspector;    // Cast to CardProspector    
            lCP.Add(tCP);    
        }    
        return lCP;    
    }
